...
(defun codex-ide-mcp-bridge--resolved-server-file ()
  "Return the emacsclient server file the bridge should target."
  (expand-file-name
   (codex-ide-mcp-bridge--resolved-server-name)
   server-auth-dir))
...
(defun codex-ide-mcp-bridge-status ()
  ...
  (let* ((enabled (codex-ide-mcp-bridge-enabled-p))
         ...
         (server-file (and enabled
                           (codex-ide-mcp-bridge--resolved-server-file)))
    ...
    (let* ((bridge-name codex-ide-emacs-tool-bridge-name)
           ...
           (emacsclient-command
            (or (executable-find codex-ide-emacs-bridge-emacsclient-command)
                codex-ide-emacs-bridge-emacsclient-command))
           (configured-server-name codex-ide-emacs-bridge-server-name)
           (script-args
            (append (list script-path
                          ...
                          "--server-file"
                          (codex-ide-mcp-bridge--resolved-server-file))
                    (when (and configured-server-name
                               (not (string-empty-p configured-server-name)))
                      (list "--server-name" configured-server-name)))))
